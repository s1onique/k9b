...
- Health assessments are `runs/health/assessments/{run_id}-{cluster}.json` and follow the same assessment schema. Triggered comparisons land under `runs/health/comparisons` along with `runs/health/triggers`, and `runs/health/history.json` tracks the previous runs that inform heuristics.
