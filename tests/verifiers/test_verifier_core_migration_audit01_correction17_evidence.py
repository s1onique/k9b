"""CORRECTION17: evidence transaction result tests.

The tests in this module validate the CORRECTION17
hardenings to the evidence transaction:

* the orchestrator derives the F17 / S17 topology from the
...
def _topology() -> RepositoryTopology:
    return RepositoryTopology(
        F16="f17f17f17f17f17f17f17f17f17f17f17f17f17f17",
        F16_tree="f17tree0000000000000000000000000000000000",
        plan_blob="f17plan0000000000000000000000000000000000",
        S16="s17s17s17s17s17s17s17s17s17s17s17s17s17s17",
        S16_tree="s17tree0000000000000000000000000000000000",
        parent_F16="parentf170000000000000000000000000000000",
        parent_S16="f17f17f17f17f17f17f17f17f17f17f17f17f17f17",
        plan_path="docs/closure-plans/ACT-K9B-VERIFIER-CORE-MIGRATION-AUDIT01-CORRECTION17.json",
    )
...
def test_c17_topology_seven_git_commands() -> None:
    """CORRECTION17: the transaction summary records exactly 7 topology
    Git commands."""
    summary = _summary(topology=7)
    assert summary.topology_git_commands == 7


def test_c17_transaction_has_no_unrecorded_git_commands() -> None:
    """CORRECTION17: ``unrecorded_git_commands`` MUST be zero."""
    summary = _summary(unrecorded=0)
    assert summary.unrecorded_git_commands == 0


def test_c17_transaction_has_no_hidden_shell_git() -> None:
    """CORRECTION17: ``hidden_shell_git_invocations`` MUST be zero."""
    summary = _summary(hidden=0)
    assert summary.hidden_shell_git_invocations == 0


def test_c17_total_git_commands_equals_recorded_count() -> None:
    """CORRECTION17: ``total_git_commands`` equals topology + range + gate."""
    summary = _summary(topology=7, range_git=3, gate=2)
    assert summary.total_git_commands == 12


def test_c17_ruff_equivalence_uses_independent_measurements() -> None:
    """CORRECTION17: the explicit and canonical diagnostics MUST be
...
def test_c17_ruff_equivalence_self_comparison_forbidden() -> None:
    """CORRECTION17: the proof MUST NOT record the same digest twice.
...
def test_c17_topology_parent_s17_equals_f17() -> None:
    """CORRECTION17: ``parent_S16`` (= parent of S17) MUST equal F17."""
    topology = _topology()
    assert topology.parent_S16 == topology.F16


def test_c17_topology_plan_path_resolves() -> None:
    """CORRECTION17: the plan path includes ``CORRECTION17``."""
    topology = _topology()
    assert "CORRECTION17" in topology.plan_path


def test_c17_evidence_publication_status_pre_rename_is_ready() -> None:
    """CORRECTION17: pre-rename ``publication_status`` is
    ``ready_to_publish``."""
    evidence = _evidence()
    assert evidence.publication_status == "ready_to_publish"


def test_c17_evidence_requires_new_fields() -> None:
    """CORRECTION17: the result dataclass has the C16+ fields."""
    evidence = _evidence()
