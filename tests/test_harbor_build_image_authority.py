...
class TestShellSafety:
    ...
    def test_preflight_env_contains_all_inputs(self) -> None:
        ...
        env = preflight_step.get("env") or {}  # type: ignore[union-attr]
    ...
    def test_preflight_uses_env_variables_not_direct_interpolation(self) -> None:
        ...
        run_body = preflight_step.get("run") or ""  # type: ignore[union-attr]
...
class TestBuildxRendering:
    ...
    def test_build_push_uses_input_condition(self) -> None:
        ...
        assert "inputs.image_push_enabled" in str(push_value), "push must use inputs.image_push_enabled"
    ...
    def test_cache_from_uses_input_condition(self) -> None:
        ...
        assert "inputs.registry_cache_read_enabled" in str(cache_from), "cache-from must use inputs.registry_cache_read_enabled"
    ...
    def test_cache_to_uses_input_condition(self) -> None:
        ...
        assert "inputs.registry_cache_write_enabled" in str(cache_to), "cache-to must use inputs.registry_cache_write_enabled"
    ...
    def test_no_unconditional_cache_to(self) -> None:
        ...
        assert "registry_cache_write_enabled" in str(cache_to), "cache-to must be conditional on registry_cache_write_enabled"
        # Must not be always-present
        assert cache_to != "type=registry,ref=..." or "registry_cache_write_enabled" in str(cache_to), "cache-to must not be unconditional"
...
class TestCallerAuthorityMatrix:
    ...
    def test_all_callers_supply_all_authorities(self) -> None:
        ...
        for job_name, inputs in calls:
            assert "image_push_enabled" in inputs, f"{job_name} must supply image_push_enabled"
            assert "registry_cache_read_enabled" in inputs, f"{job_name} must supply registry_cache_read_enabled"
            assert "registry_cache_write_enabled" in inputs, f"{job_name} must supply registry_cache_write_enabled"
    ...
    def test_pr_callers_use_read_only_matrix(self) -> None:
        ...
        for job_name, inputs in calls:
            ...
            if "image_push_enabled" in inputs:
                ...
                assert "github.event_name" in value, f"{job_name} image_push_enabled should use event condition"

            if "registry_cache_write_enabled" in inputs:
                value = str(inputs["registry_cache_write_enabled"])
                assert "github.event_name" in value, f"{job_name} registry_cache_write_enabled should use event condition"
    ...
    def test_login_condition_uses_authority_inputs(self) -> None:
        ...
        assert "image_push_enabled" in condition, "Login condition must check image_push_enabled"
        assert "registry_cache_write_enabled" in condition, "Login condition must check registry_cache_write_enabled"
...
class TestAuthorityPreflight:
    ...
    def test_has_authority_preflight_step(self) -> None:
        ...
        assert "Authority preflight" in step_names, "Must have Authority preflight step"
    ...
    def test_preflight_checks_credentials_for_write(self) -> None:
        ...
        assert "Credentials validated" in content or "credentials validated" in content.lower(), "Must report credential validation"
    ...
    def test_preflight_exits_early_for_pr(self) -> None:
        ...
        assert "exit 0" in run_body, "Preflight must exit early for read-only PR"
    ...
    def test_preflight_has_correct_phase_order(self) -> None:
        ...
        preflight_match = re.search(r"Authority preflight.*?run: \|\s*(.*?)(?=\n      - name:|\njobs:)", content, re.DOTALL)
        ...
        if preflight_match:
            ...
            assert validate_inputs_pos < validate_boolean_pos, "Phase 1 (validate inputs) must come before Phase 2 (validate booleans)"
            assert validate_boolean_pos < reject_pr_pos, "Phase 2 (validate booleans) must come before Phase 3 (reject PR)"
            assert reject_pr_pos < write_required_pos, "Phase 3 (reject PR) must come before Phase 4 (determine write)"
            assert write_required_pos < validate_creds_pos, "Phase 4 (determine write) must come before Phase 5 (validate creds)"
...
class TestPolicyTruthTable:
    ...
    def test_pr_event_image_push_false(self) -> None:
        ...
        assert "github.event_name != 'pull_request'" in content or "github.event_name == 'pull_request'" in content, "Must use event condition for image_push_enabled"
    ...
    def test_pr_event_cache_write_false(self) -> None:
        ...
        assert "github.event_name != 'pull_request'" in content or "github.event_name == 'pull_request'" in content, "Must use event condition for registry_cache_write_enabled"
    ...
    def test_pr_event_cache_read_true(self) -> None:
        ...
        assert "registry_cache_read_enabled: true" in content, "PR should have cache_read enabled (read-only)"
    ...
    def test_trusted_push_all_write_true(self) -> None:
        ...
        assert "github.event_name != 'pull_request'" in content, "Trusted push should use github.event_name != 'pull_request' for write"
