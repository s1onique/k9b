"""Unit tests for the K3s CNPG incident lab artifact verifier."""
...
class TestVerifierFailSecret:
    """Test that the verifier fails when secrets are detected in artifacts."""

    def test_verifier_fails_on_secret_leakage(self) -> None:
        """When secrets are detected in artifacts, should fail."""
        result = subprocess.run(
        ...
        assert result.returncode == 1, f"Verifier should have failed but didn't:\nstdout: {result.stdout}"
        assert "FAILED" in result.stdout
        # Should mention secret detection.
        assert "password" in result.stdout.lower() or "secret" in result.stdout.lower() or "api" in result.stdout.lower()
