...
- Integration with next_check_planner via execution context
- Signal extraction and truncation handling
"""
...
class TestBuildExecutionResultDigest(unittest.TestCase):
    ...
    def test_extracts_signal_markers(self) -> None:
        """Test that signal markers are extracted from raw output."""
        artifact = ExternalAnalysisArtifact(
            tool_name="next-check-executor",
            run_id="run-exec-signals",
            cluster_label="prod-cluster",
            status=ExternalAnalysisStatus.FAILED,
            raw_output="Pod nginx is in CrashLoopBackOff and ImagePullBackOff state",
        )
...
class TestProvenanceAttachment(unittest.TestCase):
    ...
    def test_attaches_provenance_on_description_overlap(self) -> None:
        ...
        from k8s_diag_agent.external_analysis.next_check_planner_candidates import (
            _build_execution_provenance,
            _find_execution_result_for_candidate,
        )
        ...
        result = _find_execution_result_for_candidate(
            "kubectl describe pod nginx-pod --namespace=default",
            (digest,),
        )
        
        self.assertIsNotNone(result)
        provenance = _build_execution_provenance(result)
