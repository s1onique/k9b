"""Tests for vmalert discovery integration in HealthLoopRunner.

Tests cover:
- Discovery invocation from the run pipeline
- Writing {run_id}-vmalert-sources.json artifact
...
class TestHealthLoopVmalertDiscovery(unittest.TestCase):
    """Tests for vmalert discovery in HealthLoopRunner."""
    ...
    def test_discovery_method_exists(self) -> None:
        """Test that HealthLoopRunner has the _run_vmalert_discovery method."""
        ...
        assert hasattr(runner, "_run_vmalert_discovery")
        assert callable(getattr(runner, "_run_vmalert_discovery"))

    def test_discovery_method_called(self) -> None:
        """Test that vmalert discovery method is called with correct parameters."""
        runner = HealthLoopRunner(
            config=self.config,
            available_contexts=["test-context"],
            quiet=True,
        ...
        with patch("k8s_diag_agent.health.loop_vmalert_discovery.discover_vmalerts") as discover_mock, \
            ...
            write_mock.return_value = self.output_dir / "health" / f"{self.run_id}-vmalert-sources.json"
            
            # Call the discovery method directly to verify it works
            runner._run_vmalert_discovery([mock_record], {"root": self.output_dir / "health"})
        
        # Verify discovery was called once
        discover_mock.assert_called_once()

    def test_writes_vmalert_sources_artifact(self) -> None:
        """Test that discovery writes {run_id}-vmalert-sources.json artifact."""
        runner = HealthLoopRunner(
            config=self.config,
            available_contexts=["test-context"],
            quiet=True,
        ...
        with patch("k8s_diag_agent.health.loop_vmalert_discovery.discover_vmalerts") as discover_mock, \
            ...
            inventory = VmalertSourceInventory()
            inventory.add_source(VmalertSource(
                source_id="vmalert:victoria-metrics/main",
                endpoint="http://vmalert.victoria-metrics.svc:8880",
                namespace="victoria-metrics",
                name="main",
                origin=VmalertSourceOrigin.VMALERT_CRD,
                state=VmalertSourceState.AUTO_TRACKED,
            ))
            discover_mock.return_value = inventory
            ...
            assert isinstance(written_inventory, VmalertSourceInventory)
    ...
    def test_discovery_skipped_when_no_records(self) -> None:
        ...
        with patch("k8s_diag_agent.health.loop_vmalert_discovery.discover_vmalerts") as discover_mock, \
            ...
            runner._run_vmalert_discovery([], {"root": self.output_dir / "health"})
            ...
            skipped_calls = [c for c in log_mock.call_args_list 
                           if c.kwargs.get("event") == "vmalert-discovery-skipped"]
    ...
    def test_aggregates_sources_across_multiple_targets(self) -> None:
        ...
        def mock_discover(context: str, cluster_uid: str | None = None) -> VmalertSourceInventory:
            inventory = VmalertSourceInventory()
            inventory.add_source(VmalertSource(
                source_id=f"{context}:vmalert",
                endpoint=f"http://vmalert-{context}:8880",
                origin=VmalertSourceOrigin.VMALERT_CRD,
    ...
    def test_write_failure_logged_but_non_fatal(self) -> None:
        ...
        with patch("k8s_diag_agent.health.loop_vmalert_discovery.discover_vmalerts") as discover_mock, \
            ...
            error_calls = [c for c in log_mock.call_args_list 
                         if c.kwargs.get("event") == "vmalert-sources-write-failed"]
