"""Stop-condition tests for incident diagnosis loop.

Tests:
1. High-confidence evidence-backed root-cause candidate stops the loop
2. Budget exhaustion stops the loop
3. No proposed checks stops with stop_no_checks_proposed
4. All proposed checks rejected stops with stop_no_safe_checks
5. Mutation request stops or safety-blocks
6. Low-confidence repeated no-progress state stops with stop_low_confidence_no_progress
"""
...
class TestBudgetExhausted(unittest.TestCase):
    ...
    def test_budget_exhausted_at_max_passes(self) -> None:
        ...
        from k8s_diag_agent.collect.incident_diagnosis_loop import increment_pass
