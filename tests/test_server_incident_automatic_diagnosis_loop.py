...
class TestHandlerErrorCases:
    ...
    def test_skipped_incident_returns_skipped_response(
        ...
        # Use SimpleNamespace instead of MagicMock to avoid attribute auto-creation
        # that can cause high-CPU/recursive behavior during serialization
        from types import SimpleNamespace

        mock_result = SimpleNamespace(
            skipped=True,
            eligible=False,
            eligibility_reason="incident_closed",
            skip_reason="incident_closed",
            budget_diagnostics=(),
        )
        mock_collect.return_value = mock_result
