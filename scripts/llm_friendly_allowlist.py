...
ALLOWLIST: list[tuple[str, str]] = [
    ...
    # [TEST] AUDIT01 audit reliability tests - covers the full R1-R19
    # contract matrix plus the R1/CORRECTION04 closure proof
    # (synthetic classify_pair fixtures, executable patch evidence,
    # import-aware consumer resolution across all 4 forms, skip
    # semantics, source-preservation proof, LLM-friendly boundary).
    (
        "tests/verifiers/test_verifier_core_migration_audit01.py",
        "[TEST] AUDIT01 audit reliability tests - covers R1-R19 contract matrix plus the CORRECTION04 closure proof",
    ),
    # [GENERATED] AUDIT01 closure reports - regenerated on every
    # ``audit --write`` invocation; not human-authored.  The audit
    # JSON embeds every per-symbol consumer record plus the patch
    # simulation details; the markdown is the human-facing summary
    # derived from the JSON.  Both are machine/summary artefacts.
    (
        "docs/reports/verifier-core-migration-audit01.json",
        "[GENERATED] AUDIT01 closure report - regenerated by ``audit --write``; embeds per-symbol consumer + patch evidence",
    ),
    (
        "docs/reports/verifier-core-migration-audit01.md",
        "[GENERATED] AUDIT01 closure summary - human-facing report derived from the JSON shard",
    ),
]
