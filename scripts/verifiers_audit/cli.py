...
CORRECTION11: the ``AUDIT01_SKIP_GATE`` environment variable and the
``skip_gate`` forwarding argument are removed.  Production has no
legitimate fast-skip workflow; the canonical
``gate_classification.json`` shard is owned exclusively by
:mod:`scripts.verifiers_audit.collect_r2_evidence` and the audit
object never re-runs the gate.
"""
...
def cmd_write(
    ...
    """
    # CORRECTION09/CORRECTION11: ``cmd_write`` MUST return nonzero
    # and perform zero side effects when a caller supplies a
