...
This module provides a typed boundary for reading HealthProposal files
from disk, replacing scattered ad-hoc json.loads() + from_dict() patterns.

Pilot scope: HealthProposal family only.
Do not extend without validation against this pattern.

Error handling model:
- Strict reader (read_health_proposal_artifact) raises specific exceptions:
  - OSError, json.JSONDecodeError, ValueError, TypeError, KeyError
- Optional reader (try_read_health_proposal_artifact) returns None on failure
